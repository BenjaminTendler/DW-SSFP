...
// CFP[6]   diffGradDurs
// CFP[7]   flipAngle
// CFP[8]   whichflip  (0=first, 1=second)
// CFP[9]   Noisefloor estimate
...
MACRO T SSFP_Signal(T adc, T* P, T* CFP, T* FixP, T E1, T E2, T sa, T ca)
{
  ...
  T signal = sqrt_gpu(fabs_gpu(S0*Mminus_top/Mminus_bottom)*fabs_gpu(S0*Mminus_top/Mminus_bottom)+CFP[9]*CFP[9]);
			 ...
			 T* FixP) 	// Fixed Parameters for each voxel
{
  ...
  T adc_aniso1= adc_iso*xv_1*xv_1;	// d*(pow(xv1,2))
  T adc_aniso2= adc_iso*xv_2*xv_2;	// d*(pow(xv2,2))
  ...
  T pred_signal= ((1-P[2]-P[5])*isoterm + P[2]*anisoterm_1 + P[5]*anisoterm_2);
